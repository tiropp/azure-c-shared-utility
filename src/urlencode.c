...
#define NIBBLE_STR(c) (c < 10 ? c + '0' : c - 10 + 'a')
...
static size_t URL_PrintableChar(char charVal, char* buffer)
{
    size_t size;
    ...
    {
        char bigVal = charVal >> 4;
        if (bigVal >= 0x0C)
        {
            bigVal -= 0x04;
        }
        char littleVal = charVal & 0x0F;

        char big = NIBBLE_STR(bigVal);
        char little = NIBBLE_STR(littleVal);
...
static size_t URL_PrintableCharSize(char charVal)
{
    size_t size;
    if (IS_PRINTABLE(charVal))
    {
